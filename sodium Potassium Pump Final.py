...
def main():    p = []
    ...
    #the list of square particles is initialized
    for i in range(30):
        p += [Particle(0,0,800,305, "square",False, "top", "K+")]
        
    #the list of circular particles is initialized
    for i in range(20):
    ...
    while True:
        ...
        #draws the particles in the pump
        particleInPump(p,q,k,a)
        
        #determines the pump orientation
        #and the status of the ATP
        #draws the resulting status
        whatsGoingOn(p,q,k,a)
        
        k.pumpDraw()
        
        #movement and depiction of square particles
        for j in p:
            j.move()
            j.drawParticle()
        #movement and depiction of circular particles
        for i in q:
            i.move()
            i.drawParticle()
